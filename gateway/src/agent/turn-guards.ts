...
/** The user names a tool or demands real execution; length or phrasing cannot make that a question. */
const EXPLICIT_TOOL_REQUEST = /(browser_control|\bbrowser\b.{0,12}(工具|tool)|用工具|调用工具|必须.{0,12}(调用|执行|使用).{0,8}工具|use the tool|call the tool|with (?:the )?tools?\b|actually (?:run|execute|perform))/i;
...
export function looksLikeActionRequest(input: string): boolean {
    ...
    if (text.length <= 400 && /[?？]\s*$/.test(text) && !/(帮我|你来|please)/i.test(text)) return false;
    // A long message usually carries the instruction at the top and pasted
    // material (logs, specs, a numbered procedure) below it; judge the head so
    // a detailed multi-step request still counts as an action request, while
    // a pasted log with no instruction does not.
    const head = text.slice(0, 400);
    return ACTION_REQUEST.test(head) || EXPLICIT_TOOL_REQUEST.test(text.slice(0, 1200));
}
...
/**
 * Identity of an action whose repetition after a "not complete" audit is a
 * restart rather than progress: navigating/typing/clicking the same target
 * in a browser, writing or deleting the same file, spawning the same
 * process. Reads and inspections (snapshot, list, status) return null — they
 * are always fine to repeat.
 */
export function replayableActionKey(tool: string, args: unknown): string | null {
    const name = (tool || '').toLowerCase();
    const a = (args && typeof args === 'object' ? args : {}) as Record<string, unknown>;
    const action = String(a.action || '').toLowerCase();
    if (name === 'browser_control' || name === 'browser') {
        if (['navigate', 'type', 'click', 'double_click', 'right_click', 'drag', 'select_option', 'press_key'].includes(action)) {
            return `${name}:${action}:${stableJson({ ...a, action: undefined, tab: undefined, timeoutSeconds: undefined })}`;
        }
        return null;
    }
    if (name === 'filesystem') {
        if (['write', 'delete', 'move', 'copy', 'mkdir', 'append'].includes(action)) {
            return `filesystem:${action}:${normalizePath(a.path)}:${stableJson({ ...a, action: undefined, path: undefined })}`;
        }
        return null;
    }
    if (name === 'process') {
        if (['spawn', 'run', 'restart', 'start'].includes(action)) {
            return `process:${action}:${normalizePath(a.cwd)}:${String(a.command || a.cmd || a.name || '').replace(/\s+/g, ' ').trim().toLowerCase()}`;
        }
        return null;
    }
    return null;
