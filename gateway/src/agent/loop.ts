...
import { claimsActionPerformed, claimsLiveState, claimsToolEvidence, isMutatingToolCall, looksLikeActionRequest, looksLikeLiveStateQuestion, normalizeAnswer, readOnlyToolKey, replayableActionKey, toolCallSignature } from './turn-guards';
...
export interface AgentLoopConfig {
    ...
    /**
     * Provider for the post-answer audits (completion guard, claim-consistency
     * check). A small fast model here keeps a slow main model from timing the
     * audits out; absent, the audits run on `llm`.
     */
...
Every entry in every list is a plain sentence. Do not wrap entries in objects or nest structures inside them.`;
    ...
    // Actions that completed successfully this turn (see replayableActionKey),
    // keyed to their call number. After a "not complete" audit a model tends
    // to start the whole procedure over — re-navigating, re-typing,
    // re-clicking — instead of doing only what is missing; those repeats are
    // intercepted and answered with the existing result.
    const completedActions = new Map<string, number>();
    let replayInterceptions = 0;
    const MAX_REPLAY_INTERCEPTIONS = 3;
    ...
    const runVerificationChat = async (
        ...
        if (!verificationProgressAnnounced) {
            ...
            const auditor = config.verificationLlm ?? config.llm;
            log.info('[Audit] model', { model: auditor.getConfig?.()?.model ?? 'unknown', dedicated: !!config.verificationLlm });
            config.onToolStart?.(
        ...
        try {
            const verificationAttempts: Promise<string>[] = [
                (config.verificationLlm ?? config.llm).chat(prompt, { signal: controller.signal }),
                timeout,
            ];
...
Strictly determine whether the task is truly completed.` },
                ...
                if (guardLine.startsWith('BLOCKED')) {
                    ...
                    messages.push({
                        role: 'system',
                        content: isZh
                            ? `⚠️ 任务尚未完成（第 ${completionGuardCount} 次检查）。用户当前的有效目标："${effectiveGoal}"。\n未完成原因：${reason}${nextStepHint}\n\n重要：生成文档、给建议、列链接都不等于任务完成。${allToolCalls.length > 0 ? '本轮已经成功执行过的步骤仍然有效，它们的工具返回就在上文——不要从头重做（不要再次 navigate、重新输入、重复点击或重复提交）。只补做真正还没发生的那一步；如果缺的只是最终汇报（漏了细节、没有引用工具原文），直接根据上文的工具返回把最终答复写完整，不要再调用工具。' : ''}需要真正执行的操作必须使用本轮实际提供的工具${browserToolForRun ? `；需要网页交互时只能使用 ${browserToolForRun}` : '；本轮没有浏览器工具，不得调用未提供的工具'}。`
                            : `⚠️ Task not completed (check #${completionGuardCount}). User's current effective goal: "${effectiveGoal}".\nReason for incompletion: ${reason}${nextStepHint}\n\nImportant: Generating documents, giving suggestions, or listing links does NOT equal task completion. ${allToolCalls.length > 0 ? 'Steps that already succeeded this turn still stand and their tool results are above — do not start over (no re-navigating, re-typing, re-clicking or re-submitting). Do only the step that genuinely has not happened; if what is missing is the final report (details left out, tool output not quoted), write the complete answer now from the tool results above without calling tools again. ' : ''}Anything that still needs doing must use only tools actually provided in this run${browserToolForRun ? `; for web interaction, use only ${browserToolForRun}` : '; no browser tool is available, so do not call an unoffered one'}.`,
                    });
                    continue;
                }
            } catch (guardError) {
...
${detailedToolLog}`,
        ...
        for (let toolIndex = 0; toolIndex < plannedToolCalls.length; toolIndex++) {
            ...
            throwAgentAbortIfNeeded(config.abortSignal);
            // A provider batch is a plan, not concurrent execution. Start only
            // the current call after the steering check, so pending calls never
            // appear running or retain that state when the plan is superseded.
            // Publish the shared public intent once; later calls have their own
            // tool description without repeating the batch's commentary.
            config.onToolStart?.(
                toolIndex === 0 ? cleanContent || toolCall.name : toolCall.name,
                [toolCall],
                toolIndex === 0 ? cleanContent || undefined : undefined,
            );
            ...
            // Only after an audit pushed back, and only a few times: a genuine
            // second navigation/submission is still possible afterwards.
            const replayKey = completionGuardCount > 0 && replayInterceptions < MAX_REPLAY_INTERCEPTIONS
                ? replayableActionKey(toolCall.name, toolCall.arguments)
                : null;
            const replayOf = replayKey ? completedActions.get(replayKey) : undefined;
            try {
                if (duplicateOf !== undefined) {
                    ...
                    // The audit said "not complete" and the model is replaying
                    // an action that already succeeded this turn. Repeating it
                    // would redo side effects (a second form submission, a
                    // second navigation) without getting closer to what is
                    // missing.
                    replayInterceptions++;
                    result = {
                        success: true,
                        code: 'ACTION_ALREADY_DONE_SKIPPED',
                        data: {
                            skipped: true,
                            sameAsCall: replayOf,
                            note: `This exact action already ran successfully earlier this turn (call #${replayOf}); its result is above and still stands — it was NOT repeated. The completion check asked only for what is still missing: do not restart the procedure. If the gap is in your final answer (missing details, outputs not quoted), write the answer now from the tool results already returned. Only call tools for a step that genuinely has not happened yet.`,
                        },
                    };
                    log.warn('[Replay Guard] Skipped a repeat of a completed action after a completion audit', { tool: toolCall.name, action: (toolCall.arguments as { action?: unknown } | undefined)?.action, sameAsCall: replayOf, interceptions: replayInterceptions });
            ...
            config.onToolCall?.(toolCall, result);
            allToolCalls.push({ name: toolCall.name, args: toolCall.arguments, result });
            if (result.success && result.code !== 'ACTION_ALREADY_DONE_SKIPPED') {
                const doneKey = replayableActionKey(toolCall.name, toolCall.arguments);
                if (doneKey && !completedActions.has(doneKey)) completedActions.set(doneKey, allToolCalls.length);
            }
            if (readKey) {
