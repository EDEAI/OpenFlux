...
export function createDesignCanvasTool(options: DesignCanvasToolOptions): AnyTool {
    ...
    return {
        ...
        description:
            ...
            + '2) 调 get_selection（或 list_images）读取该槽位的 targetWidth/targetHeight/aspect 与 notes：'
            + 'notes 是叠在槽位上的文字标注，就是这个槽位要生成的画面主题，必须严格以它为 prompt 主题；'
            + 'notes 为空且用户消息也没给主题时，先问用户要生成什么，绝不能沿用历史会话里的旧主题自行发挥；\n'
            + '3) 用 generate_image 生成图片：比例以槽位 aspect 为准，尺寸选生图模型支持的最接近档位即可'
            + '（填充时画布会按槽位框自动等比裁剪贴合，不需要为精确像素反复重生成）；\n'
            ...
            + '指令优先级：连接箭头(links) 与框选/箭头说明是主指令；notes 只是补充参考——它可能是此前生成留下的旧标注，'
            + '与 links/annotations/arrows 冲突时一律以后者为准，绝不能因 notes 推翻箭头说明的任务。'
