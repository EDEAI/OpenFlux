 ...
 *   xlsx -> openpyxl -> Markdown table
 *   pptx -> python-pptx -> slide text
 *   pdf -> pdfminer.six -> plain text
 *   csv -> built-in parsing -> Markdown table
...
/** Python conversion script generated by extension */
function buildScript(ext: string, filePath: string): string {
    // Escaped paths (Windows backslash)
...
`;
        ...
        case 'pdf':
            // Try fitz (pymupdf) first, then pdfminer.six
            return `
import sys
try:
    import fitz
...
/**
 * Execute Python script to convert files
 */
...
const EXT_MODULES: Record<string, string[]> = {
    ...
    xls: ['openpyxl'],
    pptx: ['pptx'],
    pdf: [],  // Automatically select fitz(pymupdf) or pdfminer at runtime
    csv: [],
