...
export class TurnTracker {
    ...
    handleLegacyProgress(progress: AgentProgressEvent): AgentRuntimeEvent[] {
        ...
        if (progress.type === 'tool_start') {
            ...
            const hasFreshCall = toolCalls.some(toolCall =>
                !this.toolItems.has(this.scopedToolCallId(toolCall.id, progress.sourceId)));
            if (progress.llmDescription?.trim() && (toolCalls.length === 0 || hasFreshCall)) {
                const commentary = this.progressCommentary(progress.llmDescription, this.currentIteration);
                if (commentary) emitted.push(commentary);
            }
            for (const toolCall of toolCalls) {
                // Some legacy adapters report progress through tool_start.
                // A repeated id still belongs to its original action and phase;
                // it must not restart a completed action or move live work.
                if (this.toolItems.has(this.scopedToolCallId(toolCall.id, progress.sourceId))) {
                    const updated = this.updateAction(
                        toolCall.id,
                        toolCall.detail || progress.description || progress.message,
                        progress.sourceId,
                    );
    ...
    updateAction(toolCallId: string, detail?: string, sourceId?: string): AgentRuntimeEvent | undefined {
        ...
        if (this.finished || !item || item.status === 'completed' || item.status === 'failed') return undefined;
