...
## 6. 目标模式（goal）时序（2026-09-04 新增）

工作模式由 `WorkMode = 'normal' | 'plan' | 'goal'` 控制（`gateway/src/work/types.ts`）。目标模式下，用户给出一个目标，网关自动推导验收标准，把每一轮当作一个普通 turn 执行，轮末由独立审计员按工具调用日志判定验收项，未达成就自动开下一轮，直到全部通过或预算耗尽。

```
用户消息(mode=goal)
  → handleChat：goalOrchestrator.startGoal → GoalStore.create(deriving) + PlanStore.enterGoalMode
  → 记录用户原话(kind=goal_request)，以 goalId/goalRound=1 入队
executeQueuedChatTurn（run lease 内）
  → orchestrator.beginRound：startRound(CAS) → 第 1 轮 deriveGoalCriteria(LLM) → setCriteria(running)
      → 贴出「验收标准」assistant 消息(kind=goal_criteria)
  → agent 输入 = buildGoalRoundPrompt(goal, round)，workMode='goal'，iterationBudget=budget.maxRoundIterations
      系统提示注入 goalModeSystemPrompt；notify_user 与计划控制工具不可见
  → onProgress 收集 tool_result → pending.roundToolLog
  → 循环返回后 orchestrator.settleRound：finishRound → verifyGoalRound(LLM 审计，证据必须对应成功的工具调用)
      → recordVerification(折算进展计数) → tracker.goalUpdate("验收 x/y")
  → chat.complete 照常发出（每轮都有输出与总结）
handle.result.then → orchestrator.onRoundSettled → decideAfterRound
  → achieved / stop(no_progress|max_rounds|wall_clock|oscillating|verifier_unavailable|round_errors)
      → finish + renderGoalReport(kind=goal_final_report) + leaveGoalMode
  → hold（pausing/paused/cancelled）→ markPaused
  → continue → startRound(n+1) → enqueueGoalRound：写入轮次标记(kind=goal_round_marker)
      → 受信任 payload {mode:'goal', goalId, goalRound, submissionId: goal:<id>:round:<n>} → handleChat
```

关键规则：

- 目标处于 deriving/running/pausing（即目标在驱动会话）期间，任何非受信任的 `chat`（含 steer/queue/附件）返回 `chat.accepted { disposition:'goal_active' }`；客户端用 `showConfirmDialog` 询问「是否暂停目标先执行新任务」，同意则 `goal.suspend`（中断当前轮、取消排队轮、目标置 paused/user）后把消息按常规模式重发。目标 paused 期间不占用会话：新消息按常规 turn 执行（客户端与网关都会把 mode=goal 回退为 normal），目标条上的「恢复」再开下一轮。
- `goal.pause` / `goal.suspend`：立即暂停——中断进行中的轮（轮号作废）、取消排队轮次、目标置 `paused/user`，条上立刻出现「恢复」；Stop 按钮走 interrupted 路径 → `paused/stopped_by_user`；`goal.cancel` 中止审计与当前 run、取消排队轮次、模式回 normal；`goal.dismiss` 把已结束目标从会话工作态移除，重启后不再显示。
- 网关重启：`GoalOrchestrator.recoverAfterRestart` 把 deriving/running/pausing 的目标置为 `paused/gateway_restart`，进行中的轮标记 interrupted，队列里的目标轮全部取消；不自动续跑。
- 持久化：`~/.openflux/goals/<goalId>.json`（`GoalStore`），会话工作态 `<sessions>/<sessionId>.work.json` 记录 `mode` 与 `goalId`。
- 前端：`#goal-strip` 显示目标、状态、第 n/max 轮、验收 x/y，按钮暂停/恢复/取消/查看报告；目标活跃时隐藏跟进队列，模式下拉切换需先确认取消目标。

相关文件：`gateway/src/work/goal-types.ts`、`goal-store.ts`、`goal-policy.ts`、`goal-verifier.ts`、`goal-prompts.ts`、`gateway/src/gateway/goal-orchestrator.ts`、`src/chat/goal-state.ts`。测试：`gateway/test/goal-mode.test.ts` 及 `gateway/src/work/goal-*.test.ts`。
