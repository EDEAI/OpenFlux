...
function bucketSummary(verb: string, members: ActivityItemState[], live?: LiveActions): { text: string; current?: string; active: boolean } {
    ...
    // of the most recent action stays active so the line never blinks out.
    const running = members.filter(item => live.runningIds.has(item.id));
    const shown = running.length
        ? running.reduce((best, item) => (item.lastSeq > best.lastSeq ? item : best), running[0])
        : (live.latestId && members.some(item => item.id === live.latestId)
            ? members.reduce((best, item) => (item.lastSeq > best.lastSeq ? item : best), members[0])
            : undefined);
...
interface LiveActions {
    ...
    /** Most recently touched action, for the gap when nothing is in flight. */
    latestId?: string;
}
...
function liveActions(state: TurnActivityState, items: ActivityItemState[]): LiveActions | undefined {
    ...
    for (const item of items) {
        if (NARRATIVE_KINDS.has(item.kind) || item.kind === 'approval' || item.kind === 'model') continue;
        if (item.status === 'running' || item.status === 'waiting') runningIds.add(item.id);
        if (!latest || item.lastSeq > latest.lastSeq) latest = item;
    }
    return { runningIds, latestId: runningIds.size ? undefined : latest?.id };
}
